...
package com.lsn.lib.ui.widget.layout;
...
public class XUILayoutHelper implements IXUILayout {
    ...
    public XUILayoutHelper(Context context, AttributeSet attrs, int defAttr, View owner) {
        mContext = context;
        mOwner = new WeakReference<>(owner);
        mBottomDividerColor = mTopDividerColor = ThemeUtils.resolveColor(context, R.attr.xui_config_color_separator_light);
        mMode = new PorterDuffXfermode(PorterDuff.Mode.DST_OUT);
        mClipPaint = new Paint();
        mClipPaint.setAntiAlias(true);
        mShadowAlpha = ThemeUtils.resolveFloat(context, R.attr.xui_general_shadow_alpha);
        mBorderRect = new RectF();

        int radius = 0, shadow = 0;
        boolean useThemeGeneralShadowElevation = false;
        if (null != attrs || defAttr != 0) {
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.XUILayout, defAttr, 0);
        ...
        if (shadow == 0 && useThemeGeneralShadowElevation) {
            shadow = ThemeUtils.resolveDimension(context, R.attr.xui_general_shadow_elevation);
    ...
    public void setUseThemeGeneralShadowElevation() {
        mShadowElevation = ThemeUtils.resolveDimension(mContext, R.attr.xui_general_shadow_elevation);
