...
package com.lsn.lib.ui.widget.picker.widget;

import android.view.View;

import com.lsn.lib.ui.R;
import com.lsn.lib.ui.widget.picker.wheelview.WheelView;
import com.lsn.lib.ui.widget.picker.wheelview.listener.OnItemSelectedListener;
import com.lsn.lib.ui.widget.picker.widget.adapter.ArrayWheelAdapter;
import com.lsn.lib.ui.widget.picker.widget.adapter.NumericWheelAdapter;
import com.lsn.lib.ui.widget.picker.widget.listener.ISelectTimeCallback;
import com.lsn.lib.ui.widget.picker.widget.utils.ChinaDateUtils;
import com.lsn.lib.ui.widget.picker.widget.utils.LunarCalendarUtils;
...
public class WheelTime {
    ...
    public void setPicker(int year, final int month, int day, int h, int m, int s) {
        if (isLunarCalendar) {
            int[] lunar = LunarCalendarUtils.solarToLunar(year, month + 1, day);
    ...
    private void setLunar(int year, final int month, int day, boolean isLeap, int h, int m, int s) {
        ...
        wv_year.setAdapter(new ArrayWheelAdapter<>(ChinaDateUtils.getYears(startYear, endYear)));// 设置"年"的显示数据
        ...
        wv_month.setAdapter(new ArrayWheelAdapter<>(ChinaDateUtils.getMonths(year)));
        wv_month.setLabel("");

        int leapMonth = ChinaDateUtils.leapMonth(year);
        ...
        if (ChinaDateUtils.leapMonth(year) == 0) {
            ...
            wv_day.setAdapter(new ArrayWheelAdapter<>(ChinaDateUtils.getLunarDays(ChinaDateUtils.leapDays(year))));
        }
        wv_day.setLabel("");
        ...
        wv_year.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                // 判断是不是闰年,来确定月和日的选择
                wv_month.setAdapter(new ArrayWheelAdapter<>(ChinaDateUtils.getMonths(year_num)));
                if (ChinaDateUtils.leapMonth(year_num) != 0 && wv_month.getCurrentItem() > ChinaDateUtils.leapMonth(year_num) - 1) {
                ...
                int maxItem = 29;
                if (ChinaDateUtils.leapMonth(year_num) != 0 && wv_month.getCurrentItem() > ChinaDateUtils.leapMonth(year_num) - 1) {
                    if (wv_month.getCurrentItem() == ChinaDateUtils.leapMonth(year_num) + 1) {
                        wv_day.setAdapter(new ArrayWheelAdapter<>(ChinaDateUtils.getLunarDays(ChinaDateUtils.leapDays(year_num))));
                        maxItem = ChinaDateUtils.leapDays(year_num);
                    } else {
                        wv_day.setAdapter(new ArrayWheelAdapter<>(ChinaDateUtils.getLunarDays(ChinaDateUtils.monthDays(year_num, wv_month.getCurrentItem()))));
                        maxItem = ChinaDateUtils.monthDays(year_num, wv_month.getCurrentItem());
                    }
                } else {
                    wv_day.setAdapter(new ArrayWheelAdapter<>(ChinaDateUtils.getLunarDays(ChinaDateUtils.monthDays(year_num, wv_month.getCurrentItem() + 1))));
                    maxItem = ChinaDateUtils.monthDays(year_num, wv_month.getCurrentItem() + 1);
                }
        ...
        wv_month.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                int maxItem = 29;
                if (ChinaDateUtils.leapMonth(year_num) != 0 && month_num > ChinaDateUtils.leapMonth(year_num) - 1) {
                    if (wv_month.getCurrentItem() == ChinaDateUtils.leapMonth(year_num) + 1) {
                        wv_day.setAdapter(new ArrayWheelAdapter<>(ChinaDateUtils.getLunarDays(ChinaDateUtils.leapDays(year_num))));
                        maxItem = ChinaDateUtils.leapDays(year_num);
                    } else {
                        wv_day.setAdapter(new ArrayWheelAdapter<>(ChinaDateUtils.getLunarDays(ChinaDateUtils.monthDays(year_num, month_num))));
                        maxItem = ChinaDateUtils.monthDays(year_num, month_num);
                    }
                } else {
                    wv_day.setAdapter(new ArrayWheelAdapter<>(ChinaDateUtils.getLunarDays(ChinaDateUtils.monthDays(year_num, month_num + 1))));
                    maxItem = ChinaDateUtils.monthDays(year_num, month_num + 1);
                }
...
//                maxItem = endD;
    ...
    public void setLabels(String label_year, String label_month, String label_day, String label_hours, String label_mins, String label_seconds) {
        ...
        if (label_year != null) {
            ...
            wv_year.setLabel(view.getContext().getString(R.string.xui_picker_view_year));
        }
        if (label_month != null) {
            wv_month.setLabel(label_month);
        } else {
            wv_month.setLabel(view.getContext().getString(R.string.xui_picker_view_month));
        }
        if (label_day != null) {
            wv_day.setLabel(label_day);
        } else {
            wv_day.setLabel(view.getContext().getString(R.string.xui_picker_view_day));
        }
        if (label_hours != null) {
            wv_hours.setLabel(label_hours);
        } else {
            wv_hours.setLabel(view.getContext().getString(R.string.xui_picker_view_hours));
        }
        if (label_mins != null) {
            wv_minutes.setLabel(label_mins);
        } else {
            wv_minutes.setLabel(view.getContext().getString(R.string.xui_picker_view_minutes));
        }
        if (label_seconds != null) {
            wv_seconds.setLabel(label_seconds);
        } else {
            wv_seconds.setLabel(view.getContext().getString(R.string.xui_picker_view_seconds));
        }
    ...
    private String getLunarTime() {
        ...
        if (ChinaDateUtils.leapMonth(year) == 0) {
            ...
            if ((wv_month.getCurrentItem() + 1) - ChinaDateUtils.leapMonth(year) <= 0) {
                month = wv_month.getCurrentItem() + 1;
            } else if ((wv_month.getCurrentItem() + 1) - ChinaDateUtils.leapMonth(year) == 1) {
        ...
        int[] solar = LunarCalendarUtils.lunarToSolar(year, month, day, isLeapMonth);
    ...
    public void setSelectChangeCallback(ISelectTimeCallback selectChangeCallback) {
