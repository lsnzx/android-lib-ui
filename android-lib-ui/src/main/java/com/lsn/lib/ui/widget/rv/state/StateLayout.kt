...
package com.lsn.lib.ui.widget.rv.state
...
class StateLayout @JvmOverloads constructor(
    ...
    init {
        ...
        try {
            emptyLayout = attributes.getResourceId(R.styleable.StateLayout_empty_layout_1, NO_ID)
            errorLayout = attributes.getResourceId(R.styleable.StateLayout_error_layout_1, NO_ID)
            loadingLayout = attributes.getResourceId(R.styleable.StateLayout_loading_layout_1, NO_ID)
        } finally {
            attributes.recycle()
        }
    }
    ...
    fun showLoading(
        ...
        if (status == Status.LOADING) {
            onLoading?.invoke(getStatusView(Status.LOADING, tag), tag)
            return
        }
    ...
    private fun showStatus(status: Status, tag: Any? = null) {
        ...
        runMain {
            try {
                ...
                if (status == Status.EMPTY || status == Status.ERROR) {
                    retryIds?.forEach {
                        targetStatusView.findViewById<View>(it)?.throttleClick {
                            showLoading(tag = statusContainer[Status.LOADING]?.tag, silent = isNetworkingRetry && !isNetworking())
                        }
                    }
                }
                when (status) {
                    Status.EMPTY -> onEmpty?.invoke(targetStatusView, tag)
                    Status.ERROR -> onError?.invoke(targetStatusView, tag)
                    Status.LOADING -> onLoading?.invoke(targetStatusView, tag)
                    Status.CONTENT -> onContent?.invoke(targetStatusView, tag)
                }
            } catch (e: Exception) {
    ...
    private fun getStatusView(status: Status, tag: Any?): View {
        ...
        val layoutId = when (status) {
            Status.EMPTY -> emptyLayout
            Status.ERROR -> errorLayout
            Status.LOADING -> loadingLayout
            Status.CONTENT -> NO_ID
        }
        if (layoutId == NO_ID) {
            when (status) {
                Status.ERROR -> throw Resources.NotFoundException("No StateLayout errorLayout is set")
                Status.EMPTY -> throw Resources.NotFoundException("No StateLayout emptyLayout is set")
                Status.LOADING -> throw Resources.NotFoundException("No StateLayout loadingLayout is set")
                Status.CONTENT -> throw Resources.NotFoundException("No StateLayout contentView is set")
            }
        }
    ...
    fun setContent(view: View) {
        statusContainer[Status.CONTENT] = StatusInfo(view, null)
    }
