...
package com.lsn.lib.ui.widget.rv
...
import com.lsn.lib.ui.widget.rv.animation.*
import com.lsn.lib.ui.widget.rv.annotaion.AnimationType
import com.lsn.lib.ui.widget.rv.item.*
import com.lsn.lib.ui.widget.rv.listener.*
import com.lsn.lib.ui.widget.rv.utils.BRV
import com.lsn.lib.ui.widget.rv.utils.setDifferModels
 ...
 * 3. 通过接口实现来扩展功能 [com.lsn.lib.ui.widget.rv.item]
 * 4. 快速添加触摸事件(防抖点击/快速点击/长按/选择/侧滑/拖拽)
 * 5. 强大的分组/展开/折叠/粘性头部/递归深度/动画/组position [expandOrCollapse]
 * 6. 自定义列表动画 [setAnimation] 默认动画 [com.lsn.lib.ui.widget.rv.animation]
...
open class BindingAdapter : RecyclerView.Adapter<BindingAdapter.BindingViewHolder>() {
    ...
    companion object {
        ...
        @Deprecated(
            "函数优化",
            ReplaceWith("BRV.modelId", "com.lsn.lib.ui.widget.rv.utils.BRV"),
    ...
    override fun getItemId(position: Int): Long {
        return getModelOrNull<ItemStableId>(position)?.getItemId() ?: RecyclerView.NO_ID
    }
    ...
    fun setDifferModels(
        ...
        val diffResult = DiffUtil.calculateDiff(
            ProxyDiffCallback(newModels, oldModels, itemDifferCallback),
            detectMoves
        )
    ...
    fun isSameGroup(
        ...
        for (index in min(position, otherPosition) - 1 downTo 0) {
            ...
            if (item is ItemExpand && item.itemSublist?.contains(aModel) == true && item.itemSublist?.contains(
                    bModel
                ) == true
            ) {
                return true
    ...
    inner class BindingViewHolder : RecyclerView.ViewHolder {
        ...
        internal fun bind(model: Any) {
            ...
            if (dataBindingEnable && viewBinding is ViewDataBinding) {
                try {
                    ...
                    val message = "DataBinding type mismatch ...(${
        ...
        fun expand(scrollTop: Boolean = false, @IntRange(from = -1) depth: Int = 0): Int {
            ...
            var position =
                if (bindingAdapterPosition == -1) layoutPosition else bindingAdapterPosition
